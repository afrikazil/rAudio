...
def connect_mpd():
    client = MPDClient()
    client.connect("localhost", 6600)  # IP и порт MPD
    return client
...
async def cmd( websocket ):
    async for args in websocket:
        ...
        result = getattr(client, command)(*args)
...
async def main():
    async with websockets.serve( cmd, '0.0.0.0', 8181, max_size=10485760, ping_interval=None, ping_timeout=None ):
